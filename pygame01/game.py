...
class Straight_Rope(Rope):
    def update(self):
        ...
        # checks if the player and the rope cllided
        def judge(self, octo_cat):
            if(self.x > (octo_cat.x) and self.x < (octo_cat + 20)):
                return True
            else:
                return False
...
class Shooting_Star(Rope):
    ...
    def judge(self, octo_cat):
        if((self.y > (octo_cat.y) and self.y < (octo_cat.y + 20)) and (
                self.x > (octo_cat.x) and self.x < (octo_cat.x + 20))):
            return True
        else:
            return False
